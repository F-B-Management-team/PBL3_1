...
namespace PBL3.Views
{
    public partial class Table : UserControl
    {
            ...
            for(int i = 1; i < dataTable.Rows.Count; i++)
            {
                if (Convert.ToBoolean(dataTable.Rows[i].Cells["TrangThai"].Value) == true)
                {
                    if (dataTable.Rows[i].Cells["IDBan"].Value.ToString() == btnTable[i].Text.Substring(6))
                    {
                        ...
                        btnTable[i].OnPressedState.BorderColor = Color.Firebrick;
                        btnTable[i].OnPressedState.FillColor = Color.Firebrick;
                        btnTable[i].onHoverState.BorderColor = Color.Firebrick;
                        btnTable[i].onHoverState.FillColor = Color.Firebrick;
                        btnTable[i].OnIdleState.BorderColor = Color.Firebrick;
                        btnTable[i].OnIdleState.FillColor = Color.Firebrick;
                        btnTable[i].OnDisabledState.BorderColor = Color.Firebrick;
                        btnTable[i].OnDisabledState.FillColor = Color.Firebrick;
                ...
                else
                {
                    if(dataTable.Rows[i].Cells["IDBan"].Value.ToString() == btnTable[i].Text.Substring(6))
                    {
                        ...
                        btnTable[i].OnPressedState.BorderColor = Color.Yellow;
                        btnTable[i].OnPressedState.FillColor = Color.Yellow;
                        btnTable[i].onHoverState.BorderColor = Color.Yellow;
                        btnTable[i].onHoverState.FillColor = Color.Yellow;
                        btnTable[i].OnIdleState.BorderColor = Color.Yellow;
                        btnTable[i].OnIdleState.FillColor = Color.Yellow;
                        btnTable[i].OnDisabledState.BorderColor = Color.Yellow;
                        btnTable[i].OnDisabledState.FillColor = Color.Yellow;
        ...
        private void BthTable1_Click(object sender, EventArgs e)
        {
            ...
            order1.s = new Order.Status_del(LoadStatusTable);
            order1.Visible = true;
            this.SendToBack();
            order1.BringToFront();
            order1.loadFood();
